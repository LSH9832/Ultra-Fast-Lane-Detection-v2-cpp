...
public:
    ...
    std::vector<pystring> split(std::vector<std::string> delimiters) {
        std::vector<pystring> tokens;
        size_t pos = 0;
        std::string::size_type prev_pos = 0;
        while (1) 
        {
            bool find = false;
            std::string now_delimiter;
            for (std::string delimiter: delimiters)
            {
                if(str_.find(delimiter, prev_pos) != std::string::npos)
                {
                    find = true;
                    pos = str_.find(delimiter, prev_pos);
                    now_delimiter = delimiter;
                    break;
                }
            }
            if (!find) break;
            ...
            tokens.push_back(pystring(str_.substr(prev_pos, pos - prev_pos)));
            prev_pos = pos + now_delimiter.length();
        }
        if (prev_pos < str_.length()) tokens.push_back(pystring(str_.substr(prev_pos)));

        for (std::string delimiter: delimiters)
        {
            if (endswith(delimiter)) 
            {
                tokens.push_back("");
                break;
            }
    ...
    // std::vector<pystring> split(std::vector<char*> delimiters) {
    //     std::vector<pystring> tokens;
    //     size_t pos = 0;
    //     std::string::size_type prev_pos = 0;
    //     while (1) 
